...
const awayPitcherPointStatus = document.getElementById("awayPitcherPointStatus");
const awayFielderPointStatus = document.getElementById("awayFielderPointStatus");
const homePitcherPointStatus = document.getElementById("homePitcherPointStatus");
const homeFielderPointStatus = document.getElementById("homeFielderPointStatus");
...
const pitcherChangeControls = document.getElementById("pitcherChangeControls");
...
const p1DefenseSelect = document.getElementById("p1DefenseSelect");
const p2DefenseSelect = document.getElementById("p2DefenseSelect");
...
const awayPitcher2Name = document.getElementById("awayPitcher2Name");
const awayPitcher3Name = document.getElementById("awayPitcher3Name");
const homePitcherName = document.getElementById("homePitcherName");
const homePitcher2Name = document.getElementById("homePitcher2Name");
const homePitcher3Name = document.getElementById("homePitcher3Name");
...
const awayPitcher2Stats = document.getElementById("awayPitcher2Stats");
const awayPitcher3Stats = document.getElementById("awayPitcher3Stats");
const homePitcherStats = document.getElementById("homePitcherStats");
const homePitcher2Stats = document.getElementById("homePitcher2Stats");
const homePitcher3Stats = document.getElementById("homePitcher3Stats");
...
const pitchers = [
  { id: "shohei", name: "ショウヘイ", throws: "R", fastKmh: 165, rightBreak: 8, leftBreak: 3, slowChange: 7, fastChange: 3, control: 4, stuff: 7, fielding: 6, stamina: 6, cost: 7 },
  { id: "yamamoto", name: "ヤマモト", throws: "R", fastKmh: 157, rightBreak: 7, leftBreak: 3, slowChange: 6, fastChange: 7, control: 8, stuff: 7, fielding: 6, stamina: 7, cost: 7 },
  { id: "saiki", name: "サイキ", throws: "R", fastKmh: 155, rightBreak: 5, leftBreak: 2, slowChange: 10, fastChange: 6, control: 6, stuff: 6, fielding: 5, stamina: 7, cost: 5 },
  { id: "kershaw", name: "カーショウ", throws: "L", fastKmh: 148, rightBreak: 4, leftBreak: 10, slowChange: 6, fastChange: 4, control: 8, stuff: 5, fielding: 4, stamina: 4, cost: 5 },
  { id: "hikari", name: "ヒカリ", throws: "L", fastKmh: 220, rightBreak: 2, leftBreak: 3, slowChange: 9, fastChange: 1, control: 3, stuff: 6, fielding: 6, stamina: 6, cost: 8 },
  { id: "magari", name: "マガリ", throws: "R", fastKmh: 90, rightBreak: 10, leftBreak: 10, slowChange: 10, fastChange: 10, control: 10, stuff: 4, fielding: 8, stamina: 4, cost: 6 },
  { id: "imanaga", name: "イマナガ", throws: "L", fastKmh: 149, rightBreak: 3, leftBreak: 6, slowChange: 5, fastChange: 10, control: 5, stuff: 8, fielding: 5, stamina: 6, cost: 5 },
  { id: "darvish", name: "ダルビッシュ", throws: "R", fastKmh: 158, rightBreak: 7, leftBreak: 6, slowChange: 6, fastChange: 5, control: 7, stuff: 4, fielding: 5, stamina: 4, cost: 6 },
  { id: "sawamura", name: "サワムラ", throws: "R", fastKmh: 170, rightBreak: 8, leftBreak: 7, slowChange: 5, fastChange: 8, control: 6, stuff: 10, fielding: 7, stamina: 8, cost: 10 },
  { id: "miller", name: "ミラー", throws: "R", fastKmh: 169, rightBreak: 8, leftBreak: 4, slowChange: 8, fastChange: 3, control: 6, stuff: 8, fielding: 6, stamina: 3, cost: 6 },
  { id: "ootake", name: "オオタケ", throws: "L", fastKmh: 143, rightBreak: 4, leftBreak: 4, slowChange: 5, fastChange: 5, control: 9, stuff: 5, fielding: 6, stamina: 5, cost: 4 }
];

const pitchTypes = {
  normal: { key: "5", label: "直球", speedFactor: 0.87, baseKmhFactor: 0.87, color: "#ffffff", targetSpread: 30, staminaCost: 1.5 },
  slow: { key: "8", label: "遅い球", speedFactor: 0.72, baseKmhFactor: 0.73, color: "#ffe66b", targetSpread: 42, staminaCost: 1 },
  fast: { key: "2", label: "速球", speedFactor: 1.15, baseKmhFactor: 1, color: "#aee7ff", targetSpread: 24, staminaCost: 2 },
  special: { key: "0", label: "決め球", speedFactor: 1.15 * 1.15, baseKmhFactor: 1.15, color: "#ffcf70", targetSpread: 16, staminaCost: 6, abilityMultiplier: 1.5 }
};
...
const staminaTuning = {
  ...
  homerPenalty: 5,
  speedDrops: [
    { threshold: 0.7, drop: 0 },
    { threshold: 0.4, drop: 3 },
    { threshold: 0.2, drop: 7 },
    { threshold: 0, drop: 12 }
  ],
  controlSpreadBonus: 1.15,
  controlDropMax: 2.4,
  fatigueDriftMax: 44,
  bendMultiplierMin: 0.58,
  speedChangeMultiplierMin: 0.64,
...
const pitcherRoles = ["pitcher", "pitcher2", "pitcher3"];
...
const pitcherPointLimit = 20;
const defaultMenuSelection = {
  away: { pitcher: "shohei", pitcher2: "saiki", pitcher3: "magari", L: "otani", C: "ichiro", R: "suzuki" },
  home: { pitcher: "yamamoto", pitcher2: "kershaw", pitcher3: "hikari", L: "schwarber", C: "judge", R: "shuto" }
};
...
const bigOutfieldFlyDistanceScale = 0.95;
...
function getPitcherStaminaState(player = activePitcher) {
  ...
  if (percent >= 0.4) return { label: "少し疲れ", percent };
  if (percent >= 0.2) return { label: "疲労", percent };
...
function getStaminaSpeedDrop(player = activePitcher) {
  const percent = getPitcherStaminaPercent(player);
  return staminaTuning.speedDrops.find((entry) => percent >= entry.threshold)?.drop ?? 12;
}

function getStaminaChangeMultiplier(player = activePitcher, min = staminaTuning.bendMultiplierMin) {
  return clamp(1 - getStaminaFatigueScore(player) * (1 - min), min, 1);
}
...
function consumePitchStamina(player, pitch) {
  adjustPitcherStamina(player, -(pitch?.staminaCost ?? 2));
}
...
function readMenu() {
  ...
  defenseControlMode = {
    away: p1DefenseSelect?.value === "manual" ? "manual" : "auto",
    home: p2DefenseSelect?.value === "manual" ? "manual" : "auto"
  };
...
function getMenuPitcherTeamCost(team) {
  const selection = menuSelection[team];
  return pitcherRoles.reduce((total, role) => total + (findById(pitchers, selection[role]).cost ?? 5), 0);
}

function updateMenuPointStatus() {
  const awayFielderCost = getMenuTeamCost("away");
  const homeFielderCost = getMenuTeamCost("home");
  const awayPitcherCost = getMenuPitcherTeamCost("away");
  const homePitcherCost = getMenuPitcherTeamCost("home");
  const awayOver = awayFielderCost > fielderPointLimit || awayPitcherCost > pitcherPointLimit;
  const homeOver = homeFielderCost > fielderPointLimit || homePitcherCost > pitcherPointLimit;
  const isOver = awayOver || homeOver;
  renderPointStatus(awayPitcherPointStatus, "投手P", awayPitcherCost, pitcherPointLimit, awayPitcherCost > pitcherPointLimit);
  renderPointStatus(awayFielderPointStatus, "野手P", awayFielderCost, fielderPointLimit, awayFielderCost > fielderPointLimit);
  renderPointStatus(homePitcherPointStatus, "投手P", homePitcherCost, pitcherPointLimit, homePitcherCost > pitcherPointLimit);
  renderPointStatus(homeFielderPointStatus, "野手P", homeFielderCost, fielderPointLimit, homeFielderCost > fielderPointLimit);
  if (menuPointStatus) {
    menuPointStatus.textContent = "";
...
function renderPointStatus(element, label, cost, limit, isOver) {
  if (!element) return;
  element.textContent = `${label} ${cost}/${limit}`;
...
function getChooserPlayerList(kind) {
  const source = kind === "pitcher" ? pitchers : batters;
  return source
...
function isMenuPlayerUnavailable(team, role, kind, playerId) {
  if (kind === "pitcher") {
    return pitcherRoles.some((otherRole) => otherRole !== role && menuSelection[team][otherRole] === playerId);
  }
...
function renderMenuPlayerPanel(team, role) {
  ...
  if (pitcherRoles.includes(role)) {
    renderPitcherPanel(findById(pitchers, menuSelection[team][role]), panel.name, panel.stats);
    return;
  }
  ...
  return `
    <div class="stamina-row">
      ...
      <div class="stamina-track" aria-label="スタミナ ${percent}%">
        ...
        <span class="stamina-mark stamina-mark-40"></span>
        <span class="stamina-mark stamina-mark-20"></span>
      </div>
    </div>
  `;
}
...
function getStaminaClass(percent) {
  ...
  if (percent >= 0.4) return "light";
  if (percent >= 0.2) return "tired";
  return "empty";
}
...
function startGame() {
  ...
  if (getMenuPitcherTeamCost("away") > pitcherPointLimit || getMenuPitcherTeamCost("home") > pitcherPointLimit) {
    message = `投手獲得ポイントは各チーム${pitcherPointLimit}以内`;
    updateMenuPointStatus();
    return;
  }
...
function startPitch(typeKey, options = {}) {
  ...
  const effectiveControl = getStaminaAdjustedControl(activePitcher) * pitchAbilityMultiplier;
  ...
  ball.prevX = startX;
  ball.prevY = startY;
  ball.radius = typeKey === "fast" || typeKey === "special" ? 8 : 9;
  ball.active = false;
  ball.baseVx = 0;
  ball.baseVy = 0;
  ball.speedScale = 1;
  ...
  pendingPitch = { releaseTime: now + pitchWindupDuration, startX, startY, targetX, targetY, framesToPlate, speedKmh, controlMissType: controlMiss.type, staminaMistake, pitchAbilityMultiplier };
  autoPitchTimer = Number.POSITIVE_INFINITY;
  message = `${pitch.label}、モーション開始`;
  updateSidebarAbilityPanels();
}

function getPitchControlProfile(control = 5, staminaFatigue = 0) {
  const wildness = clamp((10 - (control ?? 5)) / 9, 0, 1);
  const fatigueSpread = 1 + staminaFatigue * staminaTuning.controlSpreadBonus;
  return {
    wildness,
    spread: clamp((0.72 + wildness * 2.65) * fatigueSpread, 0.72, 5.25),
    verticalSpread: clamp((0.82 + wildness * 2.35) * (1 + staminaFatigue * 0.82), 0.82, 4.6),
    wildMissChance: wildness * 0.055 + staminaFatigue * 0.02,
    mistakeChance: wildness * 0.045 + staminaFatigue * 0.018
  };
}

function getStaminaAdjustedControl(player) {
  const fatigue = getStaminaFatigueScore(player);
  return clamp((player.control ?? 5) - fatigue * staminaTuning.controlDropMax, 1, 10);
}
...
function getEffectivePitcherStuff(player = activePitcher) {
  return (player.stuff ?? 5) * getCurrentPitchAbilityMultiplier();
}
...
function applyPitchBend(direction, frameScale = 1, progress = getPitchProgress()) {
  ...
  const staminaMultiplier = getStaminaChangeMultiplier(activePitcher, staminaTuning.bendMultiplierMin);
  const ability = (0.55 + rating * 0.09) * (ball.pitchAbilityMultiplier ?? 1) * staminaMultiplier;
...
function applyPitchSpeedChange(direction, frameScale = 1) {
  ...
  const rating = (direction < 0 ? activePitcher.slowChange : activePitcher.fastChange) * (ball.pitchAbilityMultiplier ?? 1);
  const ratingEffect = Math.pow(clamp(rating, 1, 15) / 10, 2);
  const staminaMultiplier = getStaminaChangeMultiplier(activePitcher, staminaTuning.speedChangeMultiplierMin);
...
function buildContactProfile(bestHit) {
  ...
  const stuffPenalty = (getEffectivePitcherStuff(activePitcher) - 5) * 0.05;
  ...
  const sweetSpotPenalty = sweetSpotMiss * (inGoodContactZone ? 0.1 : 0.24)
    + Math.pow(sweetSpotMiss, 1.18) * lowMeetPressure * (inGoodContactZone ? 0.18 : 0.34);
  const lowMeetQualityDrag = sweetSpotMiss * lowMeetPressure * 0.11;
...
function decideOutsideZoneHitResult(profile, contact, roll) {
  ...
  if (isInsideChaseContact(contact)) {
    if (roll < 0.88 || profile.launchAngle >= 18 || quality < 0.62) {
      return makePopupFlyResultFromProfile(profile);
    }
  }
  ...
  if (profile.launchAngle >= 24) {
    ...
    if (profile.exitVelocity >= 0.82 && profile.carry >= 0.72 && roll < 0.26) return makeChaseFlyResultFromProfile(profile);
...
function decideNormalZoneHitResult(profile, contact, roll) {
  ...
  if (profile.launchAngle >= 30) {
    if (profile.fenceEdgeFlyScore > 0.28 && roll < profile.fenceEdgeFlyScore * 0.5) return makeFenceEdgeFlyResultFromProfile(profile);
    if (profile.chaseFlyScore > 0.34 && roll < profile.chaseFlyScore * 0.88) return makeChaseFlyResultFromProfile(profile);
  ...
  if (profile.launchAngle >= 29 && profile.fenceEdgeFlyScore > 0.34 && roll < profile.fenceEdgeFlyScore * 0.48) return makeFenceEdgeFlyResultFromProfile(profile);
  if (profile.launchAngle >= 29 && profile.chaseFlyScore > 0.24 && roll < profile.chaseFlyScore) return makeChaseFlyResultFromProfile(profile);
  if (profile.launchAngle >= 29 && profile.exitVelocity >= 0.74 && roll < 0.58) return makeRoutineFlyResultFromProfile(profile);
...
function decideYellowZoneHitResult(profile, contact, roll) {
  ...
  if (excellentContact && powerDriveScore >= 0.35 && profile.launchAngle >= 14 && roll > hitEase * yellowZoneHitTuning.deepDriveRollRatio) {
    return makeDeepDriveResultFromProfile(profile);
  }
  if (strongContact && powerDriveScore >= 0.18 && profile.launchAngle >= 12 && roll > hitEase * yellowZoneHitTuning.fenceRollRatio) {
    return makeFenceEdgeFlyResultFromProfile(profile);
  }
...
function buildBattedBallProfile(contact) {
  ...
  const stuffPressure = (getEffectivePitcherStuff(activePitcher) - 5) * 0.05;
...
function getRunnerSpeed(runnerInfo) {
  return (runnerSpeedBaseRun + getEffectiveRunRating(runnerInfo?.run ?? 5)) * runnerSpeedUnit;
}

function getEffectiveRunRating(runRating) {
  const run = clamp(runRating ?? 5, 1, 10);
  const highRunCompression = 0.775;
  return 1 + (run - 1) * highRunCompression;
}
...
function getAggressiveRunnerScore(runner, battedBall, outcome) {
  ...
  const runnerBoost = ((runner.run ?? 5) - 5) * 0.085;
  ...
  const shortHitPenalty = outcome?.scoreType === "single" && depthRatio < 0.34 ? 0.16 : 0;
  return depthRatio * 0.84 + runnerBoost + ballTimeBoost + trajectoryBoost + powerBoost - shortHitPenalty;
}
...
function canBatterRunnerBeatThrowToSecond(outcome, battedBall, fieldingTarget, fielder, runner) {
  ...
  const defenseTimeToSecond = fieldingTime + getAutoThrowSetSeconds(fielder) + throwProfileToSecond.throwTime;
...
function canBatterRunnerTargetBase(runner, targetBase) {
  ...
  if (targetBase === "first") return runner.currentBase !== "home" || runner.targetBase !== "first" || !runner.arrived;
  const targetIndex = getBatterRunnerTargetIndex(targetBase);
  const currentIndex = getBatterRunnerPhysicalBaseIndex(runner);
...
function handleBatterRunnerBaseCommand(targetBase) {
  ...
  const legalTargetBase = getBatterRunnerNoPassTargetBase(targetBase, elapsedSeconds);
  if (!canBatterRunnerTargetBase(runner, legalTargetBase)) return;
  setBatterRunnerManualDestination(runner, legalTargetBase, elapsedSeconds);
  advanceForcedBaseRunnersForBatterTarget(legalTargetBase, elapsedSeconds);
  retargetDefenseThrowToBatterRunner(elapsedSeconds);
  refreshDefenseThrowSafety();
  message = `${getBaseLabel(legalTargetBase)}へ走塁指示`;
}
...
function retargetDefenseThrowToBatterRunner(elapsedSeconds = 0) {
  ...
  if (baseHoldingBall) {
    ...
    if (runner.targetBase === baseHoldingBall && defenseState.throw) {
...
function canManualDefenseThrow(targetBase) {
  ...
  const elapsedSeconds = (performance.now() - defenseState.startTime) / 1000;
  if (elapsedSeconds < defenseState.throw.prepareStartTime) return false;
  if (!Number.isFinite(defenseState.throw.startTime)) {
    return elapsedSeconds < defenseState.throw.holdDeadline || !isBatterRunnerSettledForResolution();
  }
  if (elapsedSeconds < defenseState.throw.startTime) return true;
...
function createThrowState(fielder, fieldingTarget, outcome, runner, options = {}) {
  ...
  const prepareStartTime = options.prepareStartTime ?? outcome.fieldingTime;
  ...
  return {
    ...
    targetBase,
    safe: Number.isFinite(throwArrivalTime) ? isDefenseThrowSafeAtBase(targetBase, throwArrivalTime, runner) : true
  };
}
...
function isDefenseThrowSafeAtBase(targetBase, throwArrivalTime, batterRunner = null) {
  ...
  if (hasRunnerAlreadySafeAtThrowTarget(throwState, batterRunner)) return true;
  const runnerArrival = getDefenseThrowTargetRunnerArrival(targetBase, batterRunner);
...
function refreshDefenseThrowSafety() {
  ...
  if (hasRunnerAlreadySafeAtThrowTarget(throwState, defenseState.runner)) {
    if (throwState) throwState.safe = true;
    return;
  }
  if (defenseState.heldBallBase && hasRunnerTargetingThrowBase(defenseState.heldBallBase, defenseState.runner)) {
    if (throwState) throwState.safe = !hasRunnerOutAtThrowTarget(throwState, defenseState.runner);
    return;
  }
  if (!throwState || !Number.isFinite(throwState.endTime)) return;
  if (hasRunnerOutAtThrowTarget(throwState, defenseState.runner)) {
    throwState.safe = false;
    return;
  }
  throwState.safe = isDefenseThrowSafeAtBase(throwState.targetBase, throwState.endTime, defenseState.runner);
}
...
function hasRunnerTargetingThrowBase(targetBase, batterRunner = null) {
  return getDefenseThrowTargetRunners(targetBase, batterRunner).length > 0;
}

function hasRunnerAlreadySafeAtThrowTarget(throwState, batterRunner = null) {
  if (!throwState?.targetBase) return false;
  return getDefenseThrowTargetRunners(throwState.targetBase, batterRunner)
    .some((runner) => isDefenseRunnerAlreadySafeAtThrowTarget(throwState, runner));
}

function hasRunnerOutAtThrowTarget(throwState, batterRunner = null) {
  if (!throwState?.targetBase) return false;
  return getDefenseThrowTargetRunners(throwState.targetBase, batterRunner)
...
function getDefenseThrowTargetRunnerArrival(targetBase, batterRunner = null) {
  const arrivals = getDefenseThrowTargetRunners(targetBase, batterRunner)
    .map((runner) => runner.arrivalTime)
    .filter(Number.isFinite);
  return arrivals.length ? Math.min(...arrivals) : null;
}

function getDefenseThrowTargetRunners(targetBase, batterRunner = null) {
  const runners = [];
  if (batterRunner?.targetBase === targetBase) runners.push(batterRunner);
  defenseState.baseRunners?.forEach((runner) => {
    if (runner.targetBase === targetBase || runner.manualTargetBase === targetBase) runners.push(runner);
  });
  return runners;
}
...
function buildBattedBall(power, direction, label) {
  ...
  const rawDistance = isPopupFly
    ...
    : isFenceEdgeFly
    ? fenceDistance + randomBetween(-260, 158)
    : isDeepDrive
    ? fenceDistance + randomBetween(-310, 188) + Math.max(0, power - 1.35) * 128 + Math.max(0, power - 2.08) * 610
    : isToweringFly
...
function getPossibleHomeRunFlightDistance(distance, fenceTravelDistance, traits = {}) {
  ...
  if (traits.isFenceEdgeFly) {
    return fenceTravelDistance + clamp(overFenceDistance * 0.95, 10, 158);
  }
...
function isPitcherHandledGrounder(battedBall) {
  return battedBall.isGrounder && !isDeepOutfieldBall(battedBall) && (
    battedBall.landingDistance < 1080 || (battedBall.power < 0.78 && battedBall.landingDistance < 1260)
  );
}
...
function isCatchableOutfieldFly(battedBall, fielder, fielderTime, ballTime) {
  ...
  const grace = 0.5 + (fielder.fielding ?? 5) * 0.045 + (battedBall.isRoutineFly ? 0.3 : 0) + (battedBall.isChaseFly ? 0.12 : 0);
...
function shouldResolveDefensePlayNow(elapsedSeconds) {
  ...
  if (outcome.kind === "out" && outcome.caught && !outcome.needsThrow) {
    return elapsedSeconds >= outcome.fieldingTime + defenseThrowResultHoldSeconds
...
function finishDefensePlay() {
  ...
  if (outcome.kind === "force") {
    if (defenseState.throw && !defenseState.throw.safe) {
      ...
      const runs = defenseState.runner?.manualControlled && defenseState.runner.targetBase !== "first"
    ...
    if (scoreType === "homer") adjustPitcherStamina(defendingPitcher, -staminaTuning.homerPenalty);
...
window.addEventListener("keydown", (event) => {
  ...
  if (isPlayerPitching() && event.key === "0") startPitch("special");
});
