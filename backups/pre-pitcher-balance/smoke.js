...
const lineupState = JSON.parse(runInGame(
  context,
  "JSON.stringify({ teams: Object.keys(selected), awayRoles: selected.away.batters.map((entry) => entry.role), homeRoles: selected.home.batters.map((entry) => entry.role), awayPitchers: selected.away.pitchers.map((entry) => entry.id), homePitchers: selected.home.pitchers.map((entry) => entry.id), awayActivePitcher: selected.away.activePitcherId })"
));
...
const rosterAndPointState = JSON.parse(runInGame(
  ...
  `(() => {
    ...
    menuSelection.away = { ...originalAway, L: "otani", C: "ichiro", R: "ruth" };
    updateMenuPointStatus();
    const overLimitDisabled = startButton.disabled;
    const overLimitText = awayFielderPointStatus.textContent;
    menuSelection.away = originalAway;
    updateMenuPointStatus();
    menuSelection.away = { ...originalAway, pitcher: "sawamura", pitcher2: "shohei", pitcher3: "yamamoto" };
    updateMenuPointStatus();
...
assert(rosterAndPointState.shohei.stuff === 7, "Shohei stuff should match the pitcher roster table");
assert(rosterAndPointState.shohei.stamina === 6, "Shohei stamina should match the pitcher roster table");
assert(rosterAndPointState.shohei.cost === 7, "Shohei pitcher cost should match the pitcher roster table");
assert(rosterAndPointState.sawamura.fastKmh === 170, "Sawamura fastball should match the pitcher roster table");
assert(rosterAndPointState.sawamura.rightBreak === 8, "Sawamura right break should match the pitcher roster table");
assert(rosterAndPointState.sawamura.leftBreak === 7, "Sawamura left break should match the pitcher roster table");
assert(rosterAndPointState.sawamura.slowChange === 5, "Sawamura slow change should match the pitcher roster table");
assert(rosterAndPointState.sawamura.control === 6, "Sawamura control should match the pitcher roster table");
assert(rosterAndPointState.sawamura.stamina === 8, "Sawamura stamina should match the pitcher roster table");
assert(rosterAndPointState.sawamura.cost === 10, "Sawamura pitcher cost should match the pitcher roster table");
assert(rosterAndPointState.miller.throws === "R", "Miller should be a right-handed pitcher");
assert(rosterAndPointState.miller.fastKmh === 169, "Miller fastball should match the pitcher roster table");
assert(rosterAndPointState.miller.rightBreak === 8, "Miller right break should match the pitcher roster table");
assert(rosterAndPointState.miller.leftBreak === 4, "Miller left break should match the pitcher roster table");
...
assert(rosterAndPointState.miller.cost === 6, "Miller pitcher cost should match the pitcher roster table");
...
assert(rosterAndPointState.ootake.control === 9, "Otake control should match the pitcher roster table");
assert(rosterAndPointState.ootake.stuff === 5, "Otake stuff should match the pitcher roster table");
assert(rosterAndPointState.ootake.fielding === 6, "Otake fielding should match the pitcher roster table");
assert(rosterAndPointState.ootake.stamina === 5, "Otake stamina should match the pitcher roster table");
assert(rosterAndPointState.ootake.cost === 4, "Otake pitcher cost should match the pitcher roster table");
...
assert(rosterAndPointState.pitcherPointLimit === 20, "pitcher point limit should be 20 per team");
assert(rosterAndPointState.defaultAwayPitcherCost === 18, "default away pitchers should fit under the pitcher point limit");
assert(rosterAndPointState.defaultHomePitcherCost === 20, "default home pitchers should fit exactly at the pitcher point limit");
...
assert(rosterAndPointState.pitcherOverLimitDisabled === true, "teams over 20 pitcher points should not be startable");
assert(rosterAndPointState.pitcherOverLimitText.includes("投手P"), "menu should show the pitcher point total");
...
assert(rosterAndPointState.pitcherChooserHtml.includes("cost-badge"), "pitcher chooser should show cost as a separate badge");
...
assert(rosterAndPointState.firstPitcherOption === "sawamura", "pitcher chooser should sort candidates by cost descending");
...
assert(rosterAndPointState.pitcherCardHtml.includes("cost-badge"), "pitcher cards should show cost as a separate prominent badge");
...
const pitcherStaminaState = JSON.parse(runInGame(
  ...
  `(() => {
    ...
    const tiredBend = getStaminaChangeMultiplier(pitcherInfo, staminaTuning.bendMultiplierMin);
...
assert(pitcherStaminaState.max === 98, "stamina rating seven should create 98 current-stamina points");
assert(pitcherStaminaState.initial === pitcherStaminaState.max, "selected pitchers should begin at full stamina");
assert(Math.abs(pitcherStaminaState.initial - pitcherStaminaState.afterFast - pitcherStaminaState.fastCost) < 0.001, "fastballs should consume two stamina points");
...
assert(Math.abs(pitcherStaminaState.beforeSpecial - pitcherStaminaState.afterSpecial - 6) < 0.001, "special pitches should consume six stamina points");
assert(pitcherStaminaState.specialCost === 6, "special pitch stamina cost should be six");
assert(Math.abs(pitcherStaminaState.specialPendingMultiplier - 1.5) < 0.001, "special pitches should carry the 1.5x ability multiplier");
assert(Math.abs(pitcherStaminaState.specialSpeedFactor - 1.3225) < 0.001, "special pitches should be fifteen percent faster than fastballs");
assert(Math.abs(pitcherStaminaState.specialBaseKmhFactor - 1.15) < 0.001, "special pitch displayed speed should be fifteen percent above the fastball base");
assert(pitcherStaminaState.tiredState === "疲労", "35 percent stamina should be marked as tired");
assert(pitcherStaminaState.tiredSpeedDrop === 7, "20-40 percent stamina should drop speed by seven km/h");
assert(pitcherStaminaState.tiredControl < 8, "fatigue should lower effective control");
assert(pitcherStaminaState.tiredBend < 1, "fatigue should reduce pitch movement strength");
...
assert(pitcherStaminaState.exhaustedState === "バテバテ", "below twenty percent stamina should be marked exhausted");
assert(pitcherStaminaState.exhaustedSpeedDrop === 12, "below twenty percent stamina should drop speed by twelve km/h");
...
assert(pitcherStaminaState.barHtml.includes("stamina-mark-40"), "stamina bar should show the forty-percent fatigue mark");
assert(pitcherStaminaState.barHtml.includes("stamina-mark-20"), "stamina bar should show the twenty-percent fatigue mark");
...
assert(Math.abs(battingTighteningState.scoreSweetSpotHalfWidth - battingTighteningState.visualSweetSpotHalfWidth) < 0.000001, "scored sweet spot should match the visible sweet spot");
assert(battingTighteningState.scoreAtVisibleEdge === 0, "visible sweet-spot edge should be the end of the scored sweet spot");
assert(battingTighteningState.scoreOutsideVisibleEdge === 0, "balls outside the visible sweet spot should not receive sweet-spot scoring");
...
assert(Math.abs(batJudgmentTrimState.rightStartTrim - 55) < 0.001, "right batter inside end should be trimmed by fifty-five percent");
assert(Math.abs(batJudgmentTrimState.rightEndTrim) < 0.001, "right batter outside end should reach full outside length");
assert(Math.abs(batJudgmentTrimState.leftStartTrim) < 0.001, "left batter outside end should reach full outside length");
assert(Math.abs(batJudgmentTrimState.leftEndTrim - 55) < 0.001, "left batter inside end should be trimmed by fifty-five percent");
assert(Math.abs(batJudgmentTrimState.rightLength - 45) < 0.001 && Math.abs(batJudgmentTrimState.leftLength - 45) < 0.001, "bat judgment segment should keep the outside ten-percent extension");
...
assert(Math.abs(runnerSpeedState.slow - runnerSpeedState.expectedSlow) < 0.001, "run 1 should keep its existing speed");
...
assert(Math.abs(runnerSpeedState.fast - runnerSpeedState.oldFastSpeed * 0.85) < 0.001, "run 10 should be reduced to eighty-five percent of its previous speed");
assert(runnerSpeedState.effectiveRun1 === 1, "effective run rating should keep run 1 unchanged");
...
assert(Math.abs(pitchControlState.low.mistakeChance - 0.045) < 0.000001, "low-control pitchers should have another halved middle-leak chance");
assert(Math.abs(pitchControlState.low.wildMissChance - 0.055) < 0.000001, "low-control pitchers should have another halved wild-miss chance");
...
const variedBattedBallState = JSON.parse(runInGame(
  ...
  `(() => {
    ...
    const extraLateFielder = { ...lateFielder, distanceToTarget: getFielderSpeed({ speed: 5 }) * 1.76 };
    const extraLateFlyCatch = resolveDefenseOutcome(extraLateFielder, lateRoutineFly, runner);
    const visualLateFielder = { ...lateFielder, distanceToTarget: getFielderSpeed({ speed: 5 }) * 1.95 };
...
assert(variedBattedBallState.builtFenceEdgeDistanceFromFence <= 360, "shortened fence-edge balls should still land in deep outfield range");
assert(variedBattedBallState.builtFenceEdgeHeight >= 495, "fence-edge balls should have the boosted large fly-ball arc");
assert(variedBattedBallState.fenceEdgeWallHit === false, "shortened fence-edge flies should be able to drop in front of the wall");
assert(variedBattedBallState.fenceEdgeWallOver === false, "barely short fence-edge flies should stay in play");
assert(variedBattedBallState.fenceEdgeWallFlightOverFence < 0, "shortened wall-scraper misses should land in front of the fence");
assert(variedBattedBallState.fenceEdgeHomerOver === false, "shortened strong fence-edge flies should stay in play more often");
assert(variedBattedBallState.fenceEdgeHomerWallHit === true, "shortened strong fence-edge flies can still threaten the wall");
assert(variedBattedBallState.fenceEdgeHomerFlightOverFence < 0, "shortened fence-edge fly distance should stay below the fence line in this boundary case");
assert(variedBattedBallState.barelyHomerOver === false, "shortened barely-cleared fence-edge flies should now stay in play");
assert(variedBattedBallState.barelyHomerFlightOverFence < 0, "shortened barely-cleared fly distance should now fall short of the fence");
...
assert(variedBattedBallState.extraLateFlyCatch.caught === true && variedBattedBallState.extraLateFlyCatch.kind === "out", "slightly late outfielders should now catch more fly balls");
assert(variedBattedBallState.visualLateFlyCatch.caught === true && variedBattedBallState.visualLateFlyCatch.kind === "out", "visibly reached routine flies should be recorded as outs");
...
const runnerDecisionState = JSON.parse(runInGame(
  ...
  `(() => {
    ...
    const firstToThirdFirstStepSecond = runner.route[1]?.x === defenseField.bases.second.x && runner.route[1]?.y === defenseField.bases.second.y;
    ...
    const beforeFirstToSecondFirstStepFirst = beforeFirstRunner.route[1]?.x === defenseField.bases.first.x && beforeFirstRunner.route[1]?.y === defenseField.bases.first.y;
    ...
    defenseState = {
      ...
      throw: { startTime: 1, endTime: 1.5, holdDeadline: 3.5, safe: true, targetBase: "second", baseLabel: "二塁" }
    };
    ...
    defenseState = {
      ...
      battedBall: longBall,
      outcome: { kind: "force", caught: true, needsThrow: true, fieldingTime: 0.6 },
      throw: { startTime: 1, endTime: 2.4, holdDeadline: 4.4, safe: true, targetBase: "second", baseLabel: "二塁" }
    };
    handleBatterRunnerBaseCommand("second");
...
assert(runnerDecisionState.beforeFirstToSecondFirstStepFirst === true, "second-base commands before first should still route through first");
assert(runnerDecisionState.beforeFirstToSecondTouchesSecond === true, "second-base commands before first should still continue to second after first");
assert(runnerDecisionState.touchedFirstWhileRunning === true, "batter-runner should mark first base touched before advancing onward");
...
assert(baseRunnerAdvanceState.fastFirstOnThirdFromSingle === true, "fast runners from first should take third on deep singles");
