...
const pitchTypes = {
  ...
  special: { key: "0", label: "決め球", speedFactor: 1.15 * 1.1, baseKmhFactor: 1.1, color: "#ffcf70", targetSpread: 16, staminaCost: 16, stuffMultiplier: 2 }
};
...
const teamPresets = {
  ...
  dendos: {
    label: "デンドーズ",
    selection: { pitcher: "cyyoung", pitcher2: "sawamura", pitcher3: "maddux", SS: "nagashima", "2B": "sadaharu", L: "ruth", C: "bonds", R: "ichiro", CA: "johnnybench", DH: "nomura", lineupOrder: [...batterRoles] }
  }
};
...
function cloneMenuSelection(source) {
  return Object.fromEntries(teamIds.map((team) => [
    team,
    { ...source[team], [lineupOrderKey]: getSelectionLineupOrder(source[team]) }
  ]));
}
...
function applyHomeRunPitcherStaminaPenalty(player) {
  if (gameMode === "practice") return;
  adjustPitcherStamina(player, -staminaTuning.homerPenalty);
}
...
function recordResponsiblePitcherRunsAllowed(fielding, runs, responsiblePitcherIds = []) {
  if (gameMode === "practice" || !runs || runs <= 0) return;
...
function chooseWinningPitcherRecord(winner, preferredPitcherId) {
  ...
  const starterWinMinimumOuts = maxInnings >= 5 ? 9 : 3;
...
function renderMenuPlayerPanel(team, role) {
  ...
  if (pitcherRoles.includes(role)) {
    ...
    if (picker) picker.textContent = role === "pitcher" ? "先発" : role === "pitcher2" ? "控え1" : "控え2";
    return;
  }
  updateLineupCardShell(team, role);
...
function shouldAutoScheduleComputerPitch() {
  return gamePhase === "playing" && gameMode === "single" && !isPlayerPitching();
}

function scheduleNextComputerPitchAfterJudgment() {
  if (shouldAutoScheduleComputerPitch() && !isPitching && !pendingPitch && !ball.active) {
    scheduleNextPitch(computerJudgmentNextPitchDelay);
  }
}

function scheduleNextPitch(delay = 900) {
  if ((gameMode === "single" || gameMode === "practice") && !isPlayerPitching()) {
...
function declareIntentionalWalk() {
  ...
  recordCurrentPitcherStat("walksAllowed", 1);
  resetCountOnly();
...
function startPitch(typeKey, options = {}) {
  if (gamePhase !== "playing" || isPitching || pendingPitch || ball.active || (stealState.active && !stealState.resolved)) return;
...
function getComputerPitchCornerCourse(type, player = activePitcher) {
  ...
  if (type === "fast" || type === "special") {
    if (prioritizeStrike) {
      if (roll < 0.24) return { direction: 0, offset: randomBetween(-10, 10), intent: "earlyBrakeStrike" };
      if (roll < 0.5) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(32, 50) * strikeOffsetScale, intent: "awayEdge" };
      if (roll < 0.68) {
      ...
      if (roll < 0.8) return { direction: -awayFromBatter, offset: -awayFromBatter * randomBetween(28, 44) * strikeOffsetScale, intent: "backdoor" };
      if (roll < 0.92) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(26, 42) * strikeOffsetScale, intent: "frontdoor" };
      return { direction: 0, offset: randomBetween(-8, 8), intent: "showCenter" };
    }
    if (roll < 0.2) return { direction: 0, offset: randomBetween(-12, 12), intent: "earlyBrakeStrike" };
    if (roll < 0.43) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(38, 56) * strikeOffsetScale, intent: "awayEdge" };
    if (roll < 0.6) {
      const side = Math.random() < 0.5 ? -1 : 1;
    ...
    return avoidWasteBall
      ...
      : { direction: awayFromBatter, offset: awayFromBatter * randomBetween(56, 72) * strikeOffsetScale, intent: "awayBall" };
  }
  if (prioritizeStrike) {
    if (roll < 0.2) return { direction: -awayFromBatter, offset: -awayFromBatter * randomBetween(26, 44) * strikeOffsetScale, intent: "backdoor" };
    if (roll < 0.4) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(24, 42) * strikeOffsetScale, intent: "frontdoor" };
    if (roll < 0.62) return { direction: -awayFromBatter, offset: -awayFromBatter * randomBetween(28, 48) * strikeOffsetScale, intent: "acceleratingStrike" };
    if (roll < 0.8) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(28, 48) * strikeOffsetScale, intent: "frontdoor" };
    if (roll < 0.94) {
  ...
  if (roll < 0.18) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(32, 52) * strikeOffsetScale, intent: "strikeToBall" };
  if (roll < 0.4) return { direction: -awayFromBatter, offset: -awayFromBatter * randomBetween(30, 52) * strikeOffsetScale, intent: "acceleratingStrike" };
  if (roll < 0.48) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(36, 58) * strikeOffsetScale, intent: "speedEscape" };
  if (roll < 0.52) return { direction: dangerousDirection, offset: dangerousDirection * randomBetween(20, 36) * strikeOffsetScale, intent: "hbpBackdoor" };
  if (roll < 0.64) return { direction: -awayFromBatter, offset: -awayFromBatter * randomBetween(30, 50) * strikeOffsetScale, intent: "backdoor" };
  if (roll < 0.76) return { direction: awayFromBatter, offset: awayFromBatter * randomBetween(28, 46) * strikeOffsetScale, intent: "frontdoor" };
  if (roll < 0.98) {
    const side = getPreferredComputerBendDirection(player, Math.random() < 0.5 ? -1 : 1);
    return { direction: side, offset: side * randomBetween(24, 46) * strikeOffsetScale, intent: "edge" };
  }
  return { direction: 0, offset: randomBetween(-10, 10), intent: "showCenter" };
}
...
function buildComputerPitchShape(plan, player = activePitcher) {
  ...
  if (type === "fast") {
    ...
    if (brakeThenBurst || (type === "normal" && Math.random() < 0.22) || (type === "slow" && Math.random() < 0.3)) {
      addEarlyBrake(brakeThenBurst ? 1.08 : 0.86);
      addLateBurst(brakeThenBurst ? 1.16 : 0.94);
    }
    if (accelerateIntoZone || course.intent === "speedEscape" || escapeIntent || Math.random() < (type === "slow" ? 0.78 : 0.64)) {
...
function computerBendPitch() {
  if ((gameMode !== "single" && gameMode !== "practice") || isPlayerPitching() || !computerPitchPlan || !isPitching || !ball.inPitch || ball.crossedPlate) return;
...
function closeMenuGamepadOverlay() {
...
function handleGamepadButtonPresses(gamepad, team) {
  ...
  if (gamePhase === "menu") {
    ...
    if (justPressed(gamepadButtons.B)) closeMenuGamepadOverlay();
...
function resolveSteal(isOut) {
  ...
  stealState = createStealState();
  if (shouldCheckCount) checkCountEnd();
}
...
function update(delta) {
  ...
  if ((gameMode === "single" || gameMode === "practice") && gamePhase === "playing" && !isPitching && !pendingPitch && !ball.active && now > autoPitchTimer) {
...
function buildContactProfile(bestHit) {
  ...
  const stuffPenalty = (getEffectivePitcherStuff(activePitcher) - 5) * 0.05 * pitcherAbilityTuning.stuffEffectScale;
...
function buildBattedBallProfile(contact) {
  ...
  const stuffPressure = (getEffectivePitcherStuff(activePitcher) - 5) * 0.05 * pitcherAbilityTuning.stuffEffectScale;
  ...
  const centerDriveScore = inGoodContactZone
    ...
    : 0;
  const exitVelocity = clamp(0.4 + readableQuality * 0.88 + powerBoost * 1.7 + practicePitcherContactBoost * 0.72 + handednessContactBoost * 0.22 + sweetSpotCenterBoost * 0.08 + yellowDriveScore * 0.18 + centerDriveScore * 0.18 + lowPowerMastery * 0.24 - qualityDrag * 0.82, 0.12, 1.75);
  ...
  const carry = clamp(exitVelocity * (1 - spin * 0.18) + (launchAngle > 14 && launchAngle < 44 ? 0.34 : 0) + (power - 5) * 0.05 + practicePitcherContactBoost * 0.78 + handednessContactBoost * 0.18 + sweetSpotCenterBoost * 0.12 + lowPowerMastery * 0.28 + (inGoodContactZone ? 0.18 + centerDriveScore * (0.12 + Math.max(powerDriveScore, lowPowerMastery * 0.36) * 0.48) : 0) + hardLiftScore * 0.2 + yellowZoneBoost * 0.24 + yellowDriveScore * yellowZoneHitTuning.carryBoost * 0.92 - outsideZoneDrag * 0.42 - weakGrounderBias * 0.045, 0.08, 1.85);
  ...
  return {
    ...
    frontDropScore,
    lineEdgeScore,
    fenceLinerScore: clamp(fenceLinerScore + practicePitcherContactBoost * 0.32, 0, 0.9),
    chaseFlyScore,
    toweringFlyScore: clamp(toweringFlyScore + practicePitcherContactBoost * 0.36, 0, 0.96),
    fenceEdgeFlyScore: clamp((yellowZoneBoost > 0 ? yellowFenceEdgeFlyScore : inGoodContactZone ? centerFenceEdgeFlyScore : fenceEdgeFlyScore) + practicePitcherContactBoost * 0.42, 0, 0.96),
    power: clamp(exitVelocity * 0.74 + carry * 0.52 + practicePitcherContactBoost * 0.42, 0.08, deepDriveTuning.maxPower),
    readableQuality,
    sweetSpotCenterBoost,
    outsideZoneDrag,
    pitchQualityBoost,
...
function makePopupFlyResultFromProfile(profile) {
  return { ...makePopupFlyResult(clamp(profile.power, 0.3, 0.52)), direction: getFlyBallDirection(profile, 0.28), battedProfile: profile };
}

function makeRoutineFlyResultFromProfile(profile) {
  return { ...makeRoutineFlyResult(clamp(profile.power, 0.52, 0.76)), direction: getFlyBallDirection(profile, 0.42), battedProfile: profile };
}
...
function getFlyBallDirection(profile, spread = 0.32) {
  ...
  const openGrassJitter = Math.random() < 0.28
    ? (Math.random() < 0.5 ? -1 : 1) * randomBetween(0.12, 0.34)
    : 0;
  const depthJitter = randomBetween(-0.12, 0.1);
...
function finishPitch(label, kind, power = 0, timeDiff = 0, hitDirection = null, battedProfile = null) {
  ...
  if (kind === "strike") {
    ...
    stealState = createStealState();
    startDefensePlay(label, kind, power, timeDiff, hitDirection, battedProfile);
    return;
  } else if (kind === "hit") {
    stealState = createStealState();
    startDefensePlay(label, kind, power, timeDiff, hitDirection, battedProfile);
...
function addRunsToBattingTeam(runs, responsiblePitcherIds = []) {
  ...
  if (runs > 0) {
    ...
    recordResponsiblePitcherRunsAllowed(fieldingTeam(), runs, responsiblePitcherIds);
...
function getTagUpDepthThreshold(startBase) {
  const ratio = startBase === 3 ? 0.42 : startBase === 2 ? 0.52 : 0.68;
...
function startDefensePlay(label, kind, power, timeDiff, hitDirection = null, battedProfile = null) {
  ...
  const baseRunners = createDefenseBaseRunnerAnimations(outcome, battedBall, null, chosenFielder, fieldingTarget);
  const forceTargets = createForceTargetsForPlay(battedBall, outcome);
  const throwState = manualFielding
    ? null
...
function chooseDefenseFielder(fielders, battedBall) {
  ...
  return eligibleFielders.reduce((best, fielder) => {
    ...
    const candidate = { ...fielder, distanceToTarget: distance, fieldingPoint, roleFit, score };
...
function getDefenseFielderRouteTarget(fielder, battedBall) {
  if (isInfielderRole(fielder?.role) && (isPotentialInfieldRouteBall(battedBall) || isInfielderReactionRouteBall(fielder, battedBall))) {
...
function getFielderReactionDelay(fielder) {
  const fielding = clamp(fielder?.fielding ?? fielder?.speed ?? 5, 1, 10);
  return clamp(
    fielderReactionDelayTuning.base - fielding * fielderReactionDelayTuning.fieldingStep,
    fielderReactionDelayTuning.fastest,
    fielderReactionDelayTuning.slowest
  );
}
...
function resolveDefenseOutcome(fielder, battedBall, runner = null) {
  ...
  const outfielderReach = !isInfielderRole(fielder.role)
    ? isFlyBall ? defenseRangeTuning.outfielderReachBonus * 0.18 : defenseRangeTuning.outfielderReachBonus
    : 0;
  ...
  const reachScale = battedBall.isLiner && isTemporaryInfielderRole(fielder.role)
    ...
    : battedBall.grounderGap && isTemporaryInfielderRole(fielder.role)
      ...
      : isFlyBall
        ? clamp(0.16 + clamp(fielder.fielding ?? fielder.speed ?? 5, 1, 10) * 0.018, 0.18, 0.38)
      : 1;
...
function isCatchableOutfieldFly(battedBall, fielder, fielderTime, ballTime) {
  ...
  const fielding = clamp(fielder.fielding ?? fielder.speed ?? 5, 1, 10);
  const speed = clamp(fielder.speed ?? fielding, 1, 10);
  const routineBonus = battedBall.isRoutineFly ? 0.03 + fielding * 0.003 : 0;
  const chaseBonus = battedBall.isChaseFly ? 0.02 + fielding * 0.002 : 0;
  const grace = 0.045 + speed * 0.014 + fielding * 0.006 + routineBonus + chaseBonus;
...
function getManualFlyLandingCatch(elapsedSeconds) {
  ...
  return defenseState.fielders.reduce((best, fielder) => {
    ...
    const radius = getManualDefenseCatchRadius(fielder, battedBall) + (battedBall.trajectory === "fly" ? 12 : 4);
    const candidate = {
...
function getManualDefenseCatchRadius(fielder, battedBall) {
  const fielding = clamp(fielder?.fielding ?? fielder?.speed ?? 5, 1, 10);
  const base = battedBall?.isGrounder ? 40 : battedBall?.isLiner ? 32 : 34;
  return base + fielding * 2.8;
}
  ...
  if (outcome.kind === "force" || throwOut) {
    if (throwOut) {
      ...
      if (!outcome.fieldingError && outcome.kind === "force") {
        recordPitcherStat(fieldingTeam(), defendingPitcher, "hitsAllowed", 1);
      }
    ...
    if (scoringHitTypes.has(scoreType)) recordPitcherStat(fieldingTeam(), defendingPitcher, "hitsAllowed", 1);
    if (scoreType === "homer") applyHomeRunPitcherStaminaPenalty(defendingPitcher);
...
function drawFlyLandingTarget(target, battedBall) {
  const elapsedSeconds = defenseState.active ? (performance.now() - defenseState.startTime) / 1000 : 0;
  const ballTime = Math.max(0.1, battedBall.ballTime ?? 1);
...
function drawHud() {
  ...
  const modeLabel = gameMode === "practice" ? "MODE: 打撃練習" : gameMode === "single" ? `MODE: 1人用 vs ${teamLabel("home")}` : "MODE: 2人用";
...
function drawPitcherGameRecordColumn(x, y, width, entries, rowHeight) {
  ...
  entries.forEach((record, index) => {
    ...
    drawFittedBoardText(`${record.name}投手 ${record.innings}イニング${formatPitcherDecisionLabels(record)}`, x, rowY + 15, width, 15, "bold");
